...
class CanonicalThirdSemanticPolicyBatchApplierTest {

    @Test
    fun applyOrPreserveThirdBatchCompletely() {
        ...
        when (existingCount) {
            ...
            EXPECTED_POLICY_COUNT -> {
                assertEquals(
                    sourcePolicySet,
                    result,
                    "An already applied third batch must be a no-op."
                )
            }

            else ->
                error(
                    "Partially applied third batch: " +
                            "$existingCount/$EXPECTED_POLICY_COUNT."
                )
        }
    ...
    private fun assertExpectedEntriesPresent(
        ...
        expectedEntries.forEach { expected ->
            assertEquals(
                expected,
                resultByIdentity[expected.identityKey]
            )
        }
    }
