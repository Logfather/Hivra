...
import de.shopme.testing.system.tools.knowledge.catalog.expansion.semantic.impact.CanonicalFirstGeneralizedSemanticPolicyWaveImpactAnalyzer
import de.shopme.testing.system.tools.knowledge.catalog.expansion.semantic.impact.CanonicalFirstGeneralizedSemanticPolicyWaveImpactBaselineFactory
import de.shopme.testing.system.tools.knowledge.catalog.expansion.semantic.impact.CanonicalFirstGeneralizedSemanticPolicyWaveImpactReference
...
class RunCanonicalFirstGeneralizedSemanticPolicyWaveImpactAnalysisTest {

    @Test
    fun analyzeFirstGeneralizedSemanticPolicyWaveImpact() {
        ...
        val expectedBaseline =
            CanonicalFirstGeneralizedSemanticPolicyWaveImpactBaselineFactory()
                .create()
        ...
        val baseline =
            if (
                ...
                require(
                    persisted == expectedBaseline
                ) {
                    "Persisted Wave-1 impact baseline has changed."
                }
        ...
        val analysis =
            CanonicalFirstGeneralizedSemanticPolicyWaveImpactAnalyzer()
                .analyze(
                    baseline = baseline,
                    currentSemanticValidation = semanticValidation,
                    currentPolicyBatches = policyBatches,
                    currentApprovedExpansion = approvedExpansion,
                    currentPolicySet = currentPolicySet
                )
    ...
    private fun printAnalysis(
        ...
        println(
            buildString {
                appendLine(
                    "First generalized semantic policy wave impact"
                )
                appendLine(
                    "---------------------------------------------"
                )
                appendLine("Analysis mode: $mode")
                appendLine("Wave: ${analysis.batchKey}")
                appendLine(
    ...
    private companion object {
        ...
        const val IMPACT_BASELINE_PATH =
            "data/generated/knowledge/catalog/expansion/impact/" +
                    "canonical-first-generalized-wave-impact-baseline.json"
