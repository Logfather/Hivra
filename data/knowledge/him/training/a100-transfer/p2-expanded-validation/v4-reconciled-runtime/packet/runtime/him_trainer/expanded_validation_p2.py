...
def _checkpoint_authority(candidate_binding: Mapping[str, Any] | None = None) -> tuple[str, str, str | None]:
    """Return the checkpoint authority selected by the packet binding.

    Historical packets retain the frozen V1 values; only the explicitly
    versioned productive checkpoint selects the productive authority.
    """
    reference = candidate_binding.get("checkpointReference") if candidate_binding else None
    digest = candidate_binding.get("checkpointLogicalDigest") if candidate_binding else None
    if reference == EXPECTED_CHECKPOINT_REFERENCE and digest == EXPECTED_CHECKPOINT_DIGEST:
        return EXPECTED_CHECKPOINT_REFERENCE, EXPECTED_CHECKPOINT_DIGEST, None
    from .evaluation_authorities import productive_v2
    if reference == productive_v2.CHECKPOINT_REFERENCE and digest == productive_v2.CHECKPOINT_DIGEST:
        return productive_v2.CHECKPOINT_REFERENCE, productive_v2.CHECKPOINT_DIGEST, productive_v2.MANIFEST_SHA256
    raise ExpandedValidationContractError("CHECKPOINT_BINDING_INVALID")
...
def _load_model_state_read_only(
    ...
    expected_ref, expected_digest, expected_manifest_sha = _checkpoint_authority(candidate_binding)
    _require(manifest.get("checkpointReference") == expected_ref, "CHECKPOINT_REFERENCE_MISMATCH")
    _require(manifest.get("checkpointLogicalDigest") == expected_digest, "CHECKPOINT_DIGEST_MISMATCH")
    if expected_manifest_sha is not None:
        _require(manifest_sha == expected_manifest_sha, "CHECKPOINT_MANIFEST_DIGEST_MISMATCH")
...
def build_dry_run(*, authority_root: str | Path, candidate_reference: str, checkpoint_reference: str, checkpoint_digest: str, execution_reference: str, historical_train_examples: Sequence[str] = (), historical_validation_examples: Sequence[str] = (), historical_train_families: Sequence[str] = (), historical_validation_families: Sequence[str] = (), historical_train_source_records: Sequence[str] = (), historical_validation_source_records: Sequence[str] = (), historical_train_review_units: Sequence[str] = (), historical_validation_review_units: Sequence[str] = ()) -> dict[str, Any]:
    ...
    _checkpoint_authority({"checkpointReference": checkpoint_reference, "checkpointLogicalDigest": checkpoint_digest})
...
def _load_packet_context(packet_root: str | Path) -> tuple[AuthorityBundle, tuple[ExpandedEvaluationExample, ...], dict[str, Any], dict[str, Any]]:
    ...
    _require(execution.get("checkpointReference") == candidate_binding.get("checkpointReference"), "EXECUTION_CHECKPOINT_BINDING_INVALID")
