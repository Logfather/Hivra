...
def checkpoint_payload(prepared: PreparedProductiveV2, run_reference: str, optimizer_step: int, state_digest: str) -> dict[str, Any]:
    payload = {"contractId": "HIM_TRAINING_CHECKPOINT_CONTRACT_V1", "version": "1", "state": "CHECKPOINT_PERSISTED", "runReference": run_reference, "optimizerStep": optimizer_step, "executionPlanReference": prepared.plan["reference"], "requestDigest": prepared.packet["request"]["logicalDigest"], "manifestDigest": prepared.packet["manifest"]["logicalDigest"], "trainingInputAuthorityDigest": prepared.packet["input_authority"]["logicalDigest"], "observedTermAuthorityDigest": prepared.packet["observed"]["logicalDigest"], "examplesDigest": prepared.packet["examples"]["logicalDigest"], "corpusDigest": prepared.packet["corpus"]["logicalDigest"], "partitionDigest": prepared.packet["partition"]["logicalDigest"], "modelId": prepared.packet["request"]["modelId"], "modelRevision": prepared.packet["request"]["modelRevision"], "tokenizerId": prepared.packet["request"]["tokenizerId"], "optimizerId": prepared.packet["request"]["optimizerId"], "stateDigest": state_digest}
...
def execute_productive_v2(prepared: PreparedProductiveV2, output_root: str | Path, model_loader: Callable[[], Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        state_digest = _sha256(_canonical({name: value.detach().cpu().tolist() for name, value in model.state_dict().items()}))
        checkpoint = checkpoint_payload(prepared, run_reference, optimizer_step, state_digest)
        atomic_json_write(output / "checkpoint.v2.json", checkpoint)
        reloaded = reload_json(output / "checkpoint.v2.json")
        _require(reloaded == checkpoint, "CHECKPOINT_RELOAD_MISMATCH")
        ...
        evidence = run_evidence_payload(prepared, run_reference, "RUN_FAILED", checkpoint, f"{type(error).__name__}:{error}")
