...
class RunHimLocalModelArtifactResolverV1Test {
    ...
    fun validExplicitArtifactsResolve() = withRoot { root ->
        ...
        assertEquals(root.toRealPath(), result.baseModel.path.parent)
        assertEquals(Path.of("base-model.artifact"), result.baseModel.relativePath)
        assertEquals(Path.of("tokenizer.artifact"), result.tokenizer.relativePath)
        assertEquals(Path.of("configuration.artifact"), result.modelConfiguration.relativePath)
    }
    ...
    fun exactlyThreeArtifactRolesAreRequired() {
        assertEquals(
            ...
            HimLocalModelArtifactResolverV1.ArtifactRole.entries.map { it.name }.toSet(),
        )
    ...
    fun nestedRegularFileResolves() = withRoot { root ->
        ...
        assertEquals(Path.of("nested", "base.artifact"), result.baseModel.relativePath)
        assertTrue(result.baseModel.path.startsWith(root.toRealPath()))
    }
    ...
    fun identicalContentsRemainExplicitlyRoleBound() = withRoot { root ->
        ...
        assertEquals(HimLocalModelArtifactResolverV1.ArtifactRole.BASE_MODEL, result.baseModel.role)
        assertEquals(HimLocalModelArtifactResolverV1.ArtifactRole.TOKENIZER, result.tokenizer.role)
        assertEquals(HimLocalModelArtifactResolverV1.ArtifactRole.MODEL_CONFIGURATION, result.modelConfiguration.role)
    }
    ...
    fun absoluteRootPathIsExcludedFromLogicalDigest() = withRoot { firstRoot ->
        withRoot { secondRoot ->
            ...
            val second = completed(validRequest(secondRoot))
            assertEquals(first.logicalDigest, second.logicalDigest)
            assertNotEquals(first.baseModel.path, second.baseModel.path)
        }
    }
    ...
    fun futureRuntimeCompatibilityIsTypedAndReadOnly() = withRoot { root ->
        val result = completed(validRequest(root))
        val fields = HimLocalModelArtifactResolverV1.Resolution::class.java.declaredFields.map { it.name }.toSet()
        assertTrue(fields.containsAll(setOf("modelBinding", "baseModel", "tokenizer", "modelConfiguration")))
        assertTrue(result.baseModel.path.isAbsolute)
        assertTrue(result.tokenizer.path.isAbsolute)
        assertTrue(result.modelConfiguration.path.isAbsolute)
    }
    ...
    fun resolvedPathsAreRealAndRootBound() = withRoot { root ->
        ...
        listOf(result.baseModel, result.tokenizer, result.modelConfiguration).forEach { artifact ->
            assertEquals(artifact.path, artifact.path.toRealPath())
            assertTrue(artifact.path.startsWith(realRoot))
            assertTrue(!artifact.relativePath.isAbsolute)
        }
    }
