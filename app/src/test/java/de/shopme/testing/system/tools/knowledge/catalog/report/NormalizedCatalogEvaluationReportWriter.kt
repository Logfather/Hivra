...
import de.shopme.testing.system.tools.knowledge.catalog.evaluation.NormalizedCatalogEvaluationResult
import java.io.File
...
class NormalizedCatalogEvaluationReportWriter(
    private val gson: Gson = createDefaultGson()
) {

    fun write(
        result: NormalizedCatalogEvaluationResult,
        outputFile: File
    ) {
        require(outputFile.name.isNotBlank()) {
            "Normalized-catalog evaluation report must have a filename."
        }

        require(!outputFile.exists() || outputFile.isFile) {
            "Normalized-catalog evaluation output path is not a file: " +
        ...
        if (!parentDirectory.exists()) {
            require(parentDirectory.mkdirs()) {
                "Could not create evaluation report directory: " +
        ...
        val content = gson.toJson(result).trimEnd() +
