...
class CatalogReviewBacklogClassificationReportWriter(
    private val gson: Gson = createDefaultGson()
) {

    fun write(
        result: CatalogReviewBacklogClassificationResult,
        outputFile: File
    ) {
        ...
        require(outputFile.name.isNotBlank()) {
            "Classification report must have a filename."
        }

        require(!outputFile.exists() || outputFile.isFile) {
            "Classification report output path is not a file: " +
        ...
        if (!parentDirectory.exists()) {
            require(parentDirectory.mkdirs()) {
                "Could not create classification report directory: " +
                        parentDirectory.absolutePath
            }
        }

        require(parentDirectory.isDirectory) {
            "Classification report parent is not a directory: " +
                    parentDirectory.absolutePath
        }
